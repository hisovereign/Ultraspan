...
def ensure_gtk():
    """Import GTK modules lazily, only once."""
    global _gtk_imported, _gtk_modules
    if not _gtk_imported:
        import gi
        gi.require_version('Gtk', '4.0')
        gi.require_version('Adw', '1')
        from gi.repository import Gtk, Gio, GLib, Adw, Gdk, GdkPixbuf
        _gtk_modules['Gtk'] = Gtk
        _gtk_modules['Gio'] = Gio
        _gtk_modules['GLib'] = GLib
        _gtk_modules['Adw'] = Adw
        _gtk_modules['Gdk'] = Gdk
        _gtk_modules['GdkPixbuf'] = GdkPixbuf
        _gtk_imported = True
    return _gtk_modules
...
# ----------------------------------------------------------------------
def get_thumbnail_path(image_path, size=120):
...
# Main Window
# ----------------------------------------------------------------------
class UltraspanWindow(ensure_gtk()['Adw'].PreferencesWindow):
    def __init__(self, app):
        ...
        self.workspace_count = 4
        self._ordered_selected_children = []
        self.selected_slots = []
    ...
    def _on_visible_page_changed(self, window, pspec):
        ...
        if self.get_visible_page() == self.status_page:
            ...
            if stdout.strip():
                # We have useful output even if the script returned non‑zero
    ...
    # ------------------------------------------------------------------
    # Pages (Using Adw.PreferencesPage)
    # ------------------------------------------------------------------
    def _build_pages(self):
        ...
        # Fixed: added "self." to these lines below
    ...
    def _on_child_clicked(self, gesture, n_press, x, y, child):
    ...
    def _get_index(self, model, val):
        for i in range(model.get_n_items()):
            if model.get_string(i) == val:
                return i
        return 0

    def _run_ultraspan(self, args):
        try:
    ...
    def _run_ultraspan_async(self, args):
        def runner():
    ...
    # ------------------------------------------------------------------
    def _populate_wallpaper_list_async(self):
        gtk = ensure_gtk()
    ...
    def _add_thumbnail_to_flowbox(self, path, pixbuf, idx):
        gtk = ensure_gtk()
        ...
        picture.set_size_request(134, 134)   # adjust thumbnail size here
    ...
    def _on_mode_toggled(self, btn):
        ...
        # Determine new max allowed selections based on active mode
        max_allowed = self._get_current_max_allowed()

        # Resize the slot array to match the active mode
        ...
        # Update visual highlight on mode buttons
        ...
        # Show/hide workspace spin
        self.workspace_select_row.set_visible(self.mode_per_workspace_btn.get_active())

        # FlowBox should never use its own selection
        self.wallpaper_flowbox.set_selection_mode(gtk['Gtk'].SelectionMode.NONE)

        self._updating_modes = False

    def _on_workspace_spin_changed(self, spin):
        pass
    ...
    def _on_multiple_files_selected(self, dialog, result):
        gtk = ensure_gtk()
        try:
    ...
    def _on_single_file_for_monitor_selected(self, dialog, result):
        gtk = ensure_gtk()
        try:
    ...
    def _update_all_widgets(self, *args):
        gtk = ensure_gtk()
        self.config.load()
    ...
    def _on_refresh_images_clicked(self, btn):
        """Clear selection and reload image list."""
        gtk = ensure_gtk()
    ...
    def _on_random_daemon_toggled(self, switch, *args):
        ...
        # Re-check the file status after half a second
        gtk['GLib'].timeout_add(500, self._update_service_status)

    def _on_refresh_daemon_toggled(self, switch, *args):
        active = switch.get_active()
        gtk = ensure_gtk()
        if active:
    ...
    def _on_refresh_now(self, btn, *args):
    ...
    def _update_service_status(self):
        """Update service switches based on daemon status."""
    ...
    def _monitor_config(self):
        gtk = ensure_gtk()
        def callback(file, other, event, data):
...
class UltraspanWindow(ensure_gtk()['Adw'].PreferencesWindow):
    ...
    def _populate_wallpaper_list_async(self):
        ...
        def load_thumbnails():
            for idx, path in enumerate(self._image_paths):
                thumb_path = get_thumbnail_path(path, size=120)
                if os.path.exists(thumb_path):
                    try:
                        pixbuf = gtk['GdkPixbuf'].Pixbuf.new_from_file(thumb_path)
                    except Exception:
                    ...
                    try:
                        pixbuf = gtk['GdkPixbuf'].Pixbuf.new_from_file_at_scale(path, 120, 120, True)
                        pixbuf.savev(thumb_path, 'png', [], [])
                    except Exception:
                        continue
                gtk['GLib'].idle_add(self._add_thumbnail_to_flowbox, path, pixbuf, idx)
    ...
    def _add_thumbnail_to_flowbox(self, path, pixbuf, idx):
        ...
        picture = gtk['Gtk'].Picture.new_for_paintable(texture)
        picture.set_can_shrink(True)
        picture.set_size_request(120, 120)   # adjust thumbnail size here
        picture.set_halign(gtk['Gtk'].Align.CENTER)
        picture.set_valign(gtk['Gtk'].Align.START)
